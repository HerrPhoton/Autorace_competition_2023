...
Нода активируется после того, как распознается знак "Туннель". Для ориентации в пространстве используется лидар и камера глубины. Оправдать такой подход можно тем, что данные препятствия находятся ниже распознавания лидара, и это дает удобство различать стены ( не путать их с препятствиями). Робот стремиться двигаться по диагонали в заданную точку. Если перед ним встречается препятствие, то, сравнивая положение объектов справа и слева, робот выбирает направление, которое ему выгоднее сделать, чтобы избежать столкновение. Если значения с лидара и камеры глубины перед объектом совпадают, то робот игнорирует этот вызов и продолжает стремиться к выходу.
